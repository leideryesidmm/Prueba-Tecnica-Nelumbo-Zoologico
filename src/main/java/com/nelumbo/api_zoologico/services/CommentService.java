...
public class CommentService {
    ...
    public List<CommentAnswerDtoRes> searchCommentsByWord(String word) {
        List<CommentDtoRes> commentDtoRes=this.repository.findCommentsByMenssageContainingIgnoreCaseAndInitialCommentIsNull(word)
                .stream()
                .map(comment -> modelMapper.map(comment, CommentDtoRes.class))
                .collect(Collectors.toList());
        return getAllCommentAnswersByComment(commentDtoRes);
    }

    public List<AnswerSearchDtoRes> searchAnswersByWord(String word) {
        return this.repository.findCommentsByMenssageContainingIgnoreCaseAndInitialCommentIsNotNull(word).stream()
